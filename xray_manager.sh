...
install_vless_direct() {
    ...
    if [[ -f ~/.acme.sh/${domain}_ecc/${domain}.key ]]; then
        ...
        echo -e "${RED}[致命错误] 无法获取证书！${PLAIN}"; return 1
    fi
...
EOF
    ...
    # 启动前最后的清理，确保端口不被死锁
    lsof -i:$port | awk '{print $2}' | grep -v PID | xargs kill -9 >/dev/null 2>&1
    ...
    if systemctl is-active --quiet xray; then
        ...
        echo -e "${YELLOW}运行 journalctl -u xray -f 查看报错详情${PLAIN}"
    fi
}
...
install_cf_tunnel() {
    ...
    if [[ "$t_choice" == "2" ]]; then
        t_port=8080
        read -p "请输入 Cloudflare 绑定的域名: " t_domain
...
EOF
    ...
    if [[ "$t_choice" == "1" ]]; then
        ...
        for i in {1..30}; do
            ...
            [[ -n "$tmp_domain" ]] && echo "$tmp_domain" > /tmp/cf_tunnel_domain && break
            sleep 1
        done
    else
...
show_node_info() {
    ...
    if [[ -f "$XRAY_CONF_DIRECT" ]]; then
        ...
        local d_alpn=$(jq -r '.inbounds[0].streamSettings.tlsSettings.alpn | join(",")' $conf | sed 's/,/%2C/g')
        echo -e "${GREEN}[节点 1: VLESS+xhttp+TLS]${PLAIN}"
...
uninstall_all() {
    ...
    rm -rf /usr/local/etc/xray /etc/systemd/system/xray.service /etc/systemd/system/xray.service.d /usr/local/bin/xray
    pkill -f cloudflared && rm -f $CF_BIN
    systemctl daemon-reload
...
main_menu() {
    while true; do
        clear
        echo -e "${CYAN}==========================================${PLAIN}"
        echo -e "${CYAN}      BoGe Xray & CF Tunnel 一键脚本      ${PLAIN}"
        echo -e "${CYAN}==========================================${PLAIN}"
        echo -e " ${YELLOW}1.${PLAIN} 安装 VLESS+xhttp+TLS"
        echo -e " ${YELLOW}2.${PLAIN} 安装 CF Tunnel"
        echo -e " ${YELLOW}3.${PLAIN} 查看当前节点信息与链接"
        echo -e " ${YELLOW}4.${PLAIN} 开启 BBR 加速"
        echo -e " ${YELLOW}5.${PLAIN} 卸载脚本及相关组件"
